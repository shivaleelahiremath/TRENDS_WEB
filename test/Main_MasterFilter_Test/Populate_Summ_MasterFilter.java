...
public class Populate_Summ_MasterFilter {
	String[]  SumMasterFilterTasks;
	int[] SumFilterStud_TargetCount;
	
	String[] SumOption_Tasks, SumFilter_Task ;
	int[] SumOption_NotTaken,SumOption_Taken, SumOption_Incomplete, Sum_TaskCount;
	int[][] SumOption_val;
	
	public void populate_Data(WebDriver driver) throws InterruptedException{
		...
		int index=0;
		for(int a=0; a<SumMasterFilterTasks.length; a++){
			String value = SumMasterFilterTasks[a].trim();
...
//					System.out.println("Target index: "+in); 
			...
			SumOption_Tasks= new String[tr_collection.size()+1];
			int[] SumPF_StudCount = new int[tr_collection.size()+1];
			SumOption_NotTaken = new int[tr_collection.size()+1];
			SumOption_Taken = new int[tr_collection.size()+1];
			for(int row = 1;row<tr_collection.size()+1;row++){
				SumOption_Tasks[row] =table_element2.findElement(By.xpath("tbody/tr["+row+"]/td[1]/bar/text")).getText();
				...
				//modified code..
				SumOption_NotTaken[row] = Integer.parseInt(No_Data) + Integer.parseInt(Incomplete);
				SumOption_Taken[row] = Integer.parseInt(Taken_Stud);
				System.out.println("Summary tab PF task: "+SumOption_Tasks[row]+" not taken: "+SumOption_NotTaken[row]+" taken count: "+SumOption_Taken[row]+" and total target student "+SumPF_StudCount[row]);
			}

			//(Modified) reading PF tasks count on the summary tab..
			int srow = tr_collection.size();
			int scolm = 7;
			SumOption_val = new int[srow][scolm];     
			for(int i=0;i < srow;i++){
				for(int j=0;j<scolm;j++){
					...
					if(Demovalue1.equalsIgnoreCase("N/A")){
						...
						SumOption_val[i][j] = Integer.parseInt(Demovalue1);
					}
					System.out.println(SumOption_val[i][j]); 			    	 
				}	
					System.out.println("-------------");
			}			
...
//			System.out.println("GENDER AND ETHNICITY table: " +table_element3.getText());
			...
			SumOption_Tasks= new String[tr_collection1.size()];
			int[] SumGender_StudCount = new int[tr_collection1.size()];
			SumOption_NotTaken = new int[tr_collection1.size()];
			SumOption_Taken = new int[tr_collection1.size()];
			try{
				for(int row = 1; row < 5-2;row++){
					SumOption_Tasks[row] = table_element3.findElement(By.xpath("tbody/tr["+(row+1)+"]/td[1]/bar/text")).getText();
					...
					//modified code..
					SumOption_NotTaken[row] = Integer.parseInt(Incomplete) + Integer.parseInt(No_Data); 
					SumOption_Taken[row] = Integer.parseInt(Taken_Stud); 
					System.out.println("Summary tab gender: " +SumOption_Tasks[row]+" not taken: "+SumOption_NotTaken[row]+" taken count: "+SumOption_Taken[row]+" and total target student "+SumGender_StudCount[row]);
				}	 
			}catch (Exception e){
			...
			for(int i=0; i<SumOption_Tasks.length; i++){ 
				if(SumOption_Tasks[i]!=null) 
					al1.add(SumOption_Tasks[i]); 
			}
			SumOption_Tasks = (String[])al1.toArray(new String[al1.size()]);
			System.out.println("summary--> "+Arrays.toString(SumOption_Tasks));

			int Genrow = tr_collection1.size();
			SumOption_val = new int[Genrow][scolm];     
			try{
				for(int i=0;i < 2;i++){
					for(int j=0;j<scolm;j++){
				...
				System.out.println("Gender not found: " +Arrays.toString(SumOption_Tasks));
			}
...
//				System.out.println("ethnitask: "+SumEthnicityTask[n].trim());
			...
			SumOption_Tasks = new String[tr_collection1.size()+1];
			SumOption_NotTaken = new int[tr_collection1.size()+1];
			SumOption_Taken = new int[tr_collection1.size()+1];
			int max1=  tr_collection1.size() + 1;
			for(int row = 0; row < max1 - 5;row++){ 
				SumOption_Tasks[row] =table_element3.findElement(By.xpath("tbody/tr["+(row+EthnIndex)+"]/td[1]/bar/text")).getText();
				...
				SumOption_NotTaken[row] = Integer.parseInt(Incomplete) + Integer.parseInt(No_Data); 
				SumOption_Taken[row] = Integer.parseInt(Taken_Stud); 
				System.out.println(SumOption_Tasks[row]+"---"+SumOption_NotTaken[row]+"---"+SumOption_Taken[row]+" and total count "+SumEthn_StudCount[row]);
			}
			...
			for(int i=0; i<SumOption_Tasks.length; i++){ 
				if(SumOption_Tasks[i]!=null)
					Sumethn.add(SumOption_Tasks[i]); 
			}
			SumOption_Tasks = (String[])Sumethn.toArray(new String[Sumethn.size()]);
			System.out.println("sum ethn task--> "+Arrays.toString(SumOption_Tasks));

			SumOption_val = new int[Genrow+1][scolm];     
			for(int i=0;i < max1-5;i++){
				for(int j=0;j<scolm;j++){
					String Demovalue1 = table_element3.findElement(By.xpath("tbody/tr["+(i+EthnIndex)+"]/td["+(j+3)+"]")).getText();
					if(Demovalue1.equalsIgnoreCase("N/A")){
						SumOption_val[i][j]= -1;
					}else{
						SumOption_val[i][j] = Integer.parseInt(Demovalue1); 
					}
					System.out.println("ethnicity: "+SumOption_val[i][j]); 			    	 
				}	
					System.out.println("------>");
			}
					
			
		/*	String[] SumGender_Ethn_Task = ObjectArrays.concat(SumOption_Tasks, SumOption_Tasks, String.class);	            
			//removes all nulls 
...
//		    System.out.println("-->"+Arrays.toString(SumGender_Ethn_Task));
		    ...
		    String[] SumFilter_Task1 = ObjectArrays.concat(SumOption_Tasks, SumGender_Ethn_Task, String.class);
