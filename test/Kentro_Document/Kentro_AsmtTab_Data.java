...
//		 wait.until(ExpectedConditions.elementToBeClickable(By.id("ui-id-10"))).click();
	...
	/*	//select the less than 90 days data on assessment tab..
		...
		*/
		...
		Thread.sleep(2000);
		for (int i=0 ;i<7;i++){
//			wait.until(ExpectedConditions.elementToBeClickable(By.xpath("html/body/div[3]/div[2]/div[1]/div/div/a[1]/span"))).click();
...
//		}
		...
		for(int i=0; i<otherTab.length;i++){
			...
			try{
				testNAValue = data1.findElement(By.xpath("tbody/tr[1]/td["+(i+17)+"]/center")).getText();
			}
			catch(Exception e){
