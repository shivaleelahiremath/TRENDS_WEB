...
public class Verify_STAAR_Maint_DocScoreDetails_TS {
	...
	public void Login(){
		...
		login.findElement(By.id("loginEmail")).sendKeys("Shivaleela@TX_TangoTrendsISD");
	...
	public void VerifyDocumentScoreDetails(String DocTitle) throws InterruptedException{			
		...
		if(RecCount.equalsIgnoreCase("0 Records")){
			...
			String PassTallyRight1 =AsmtData.findElement(By.xpath("tbody/tr["+rowCount+"]/td[11]/right")).getText();
			int PassTallyRight = Integer.parseInt(PassTallyRight1);

			String FailTallyRight1 =AsmtData.findElement(By.xpath("tbody/tr["+rowCount+"]/td[12]/right")).getText();
			int FailTallyRight = Integer.parseInt(FailTallyRight1);

			String TotalTakenStud1 = AsmtData.findElement(By.xpath("tbody/tr["+rowCount+"]/td[13]")).getText();
			int TotalTakenStud = Integer.parseInt(TotalTakenStud1.replace(",", ""));

			String TotalNonTakenStud1 = AsmtData.findElement(By.xpath("tbody/tr["+rowCount+"]/td[14]")).getText();
			...
			for(int l=0;l<AsmtTabData.length;l++){
				AsmtTabValues[l] = AsmtData.findElement(By.xpath("tbody/tr["+rowCount+"]/td["+(l+15)+"]/right")).getText();				  
			}
			...
			String Incomplete = AsmtData.findElement(By.xpath("tbody/tr["+rowCount+"]/td[22]")).getText();
			...
			for(int m=0; m <AsmtTabPerc1.length ;m++){
				String AsmtPer = AsmtData.findElement(By.xpath("tbody/tr["+rowCount+"]/td["+(m+11)+"]/left")).getText();
			...
			for(int m=0;m<AsmtTabData.length;m++){
				String AsmtPer = AsmtData.findElement(By.xpath("tbody/tr["+rowCount+"]/td["+(m+15)+"]/left")).getText();
			...
			if(SumFilter_Task.length == index){
				...
				debugLog.error("PF/Gender/Ethnicity tasks count is not matching b/w summary and student filter option for document: "+DocTitle+" task length on summary tab is "+SumFilter_Task.length+" and found task on student filter option is: "+index);
			}

			//verifying Program Funding, Gender and Ethnicity task count... 
			for(int task=0; task < StudPF_Tasks.length; task++){	    	  
				String StudTask = getFilterName(StudPF_Tasks[task].trim());
