...
public class SAccount_DEMO_TS_BISDTest03 {
	...
	String sheetPath = "test/Resources/Data/BISDTest03.xls";	
    WebDriver login;
	...
	public void setUp() throws InterruptedException{
		...
		debugLog.error("launched demo / summary tab data...");
	...
	public void Login(){
		...
		debugLog.error("login successfull..");
	}
	...
	public void getExcelData() throws Exception {
		...
		for(int i= 0 ;i<sht.getRows(); i++){
			...
			String AdminMode = sht.getCell(1, i).getContents();
			String AdminSeries = sht.getCell(2, i).getContents();
			//	System.out.println("data - > "+DocTitle+"   "+AdminMode+"   "+AdminSeries);
			VerifyProgramFunInfo(DocTitle, AdminMode, AdminSeries);
		}		
	}

	public  void VerifyProgramFunInfo(String DocTitle, String AdminMode, String AdminSeries) throws InterruptedException{
	...
	//Selecting STAAR tab on assessment module..
		...
		WebElement DemoData11 = Demo1.get(2);	       
		//System.out.println("assessment data: "+DemoData.getText());

		//Filtering and selecting Program Funding details.. 
		WebElement web11 = DemoData11.findElement(By.xpath("tr/th[6]/filter/div"));
		WebElement Group11= web11.findElement(By.tagName("input"));
		Group11.clear();
		Group11.sendKeys(AdminMode);
		Group11.sendKeys(Keys.ENTER);
		Thread.sleep(2000);

		WebElement web1 = DemoData11.findElement(By.xpath("tr/th[5]/filter/div"));
				...
				for(int j=1; j < SumGender.length+1; j++){	
					...
					if(DemoGenderTask.equalsIgnoreCase(SumGenderTask)){
						if(Arrays.equals(GenderRowValue1[i-1], SumGender_val[j-1])){
							...
							debugLog.error("Gender -> "+DemoGenderTask+" student count mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries);
						}
						if(DemoGen_NotTakenCount == SumGen_NotTakenCount){
							...
							debugLog.error("Gender -> "+DemoGenderTask+" Not taken student count mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+" Expected value on Student tab is "+DemoGen_NotTakenCount+" and found value on summary tab is: "+SumGen_NotTakenCount);
						}
						if(DemoGen_TakenCount == SumGen_TakenCount){
							...
							debugLog.error("Gender -> "+SumGenderTask+" taken student count is mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+ " Expected value on Student tab is "+DemoGen_TakenCount+" and found value on summary tab is: "+SumGen_TakenCount);
						}
					}
				}
			}
				...
				for(int j=0;j<SumEthnicity.length;j++){
					...
					if(DemoEthTask.equalsIgnoreCase(SumEthTask)){	
						if(Arrays.equals(EthnicityRowValue1[i], SumEthnicity_val[j])){
							...
							debugLog.error("Ethnicity -> "+DemoEthTask+" student count mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries);
						}
						if(DemoEth_NotTakenCount == SumEth_NotTakenCount){
							...
							debugLog.error("Ethnicity -> "+DemoEthTask+" Not taken student count mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+ " Expected value on Student tab is "+DemoEth_NotTakenCount+" and found value on summary tab is: "+SumEth_NotTakenCount);
						}
						if(DemoEth_TakenCount == SumEth_TakenCount){
							...
							debugLog.error("Ethnicity -> "+DemoEthTask+" taken student count mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+" Expected value on Student tab is "+DemoEth_TakenCount+" and found value on summary tab is: "+SumEth_TakenCount);
						}
					}
				}
			}
				...
				for(int j=1; j < SumPF_Tasks.length; j++){	
					...
					if(DemoTask.equalsIgnoreCase(SumTask)){
						...
						if(Arrays.equals(rowValue1[i-1], SumPF_val[j-1])){
							...
							debugLog.error("Program Funding -> "+DemoTask+" student count mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries);
						}		            	
						if(DemoPF_NotTakenCount == SumPF_NotTakenCount){
							...
							debugLog.error("Program Funding -> "+DemoTask+" Not taken student count mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+ " Expected value on Student tab is "+DemoPF_NotTakenCount+" and found value on summary tab is: "+SumPF_NotTakenCount);
						}
						if(DemoPF_TakenCount == SumPF_TakenCount){
							reportsLog.info("PF taken student count is matching for task: "+DemoTask+" and "+SumTask);
						}else{
							debugLog.error("Program Funding -> "+DemoTask+" taken student count mismatch between Demography and Summary tab for document: " +DocTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+ "Expected value on Student tab is "+DemoPF_TakenCount+" and found value on summary tab is: "+SumPF_TakenCount);
						}
