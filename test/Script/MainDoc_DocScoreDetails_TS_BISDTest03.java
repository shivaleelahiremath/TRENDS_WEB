...
public class MainDoc_DocScoreDetails_TS_BISDTest03 {
	...
	public void VerifyDocumentScoreDetails(String DocTitle, String AdminMode, String AdminSeries) throws InterruptedException{			
		...
		login.findElement(By.xpath("html/body/content/div[1]/div[2]/div/div/div/div[1]/div/table/thead/tr/th[7]/main/left")).click();
		Thread.sleep(2000);
		...
		if(RecCount.equalsIgnoreCase("0 Records")){
			...
			for (int temp=0;temp<AsmtTabPerValues.length;temp++){			  
				...
				if(PercDiff > 1.0){
					debugLog.error(AsmtTabPerc[temp]+ " Percentage is not matching b/w summary and assessment tab for document: "+DocumentTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+" and difference value is: "+PercDiff);
				}else{
			...
			if(PassPerc> 1.0){
				debugLog.error("Pass Percentage is not matching b/w summary and assessment tab for document: "+DocumentTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+" and difference value is: "+PassPerc); 
			}else{
			...
			if(FailPerc > 1.0){
				debugLog.error("Fail Percentage is not matching b/w summary and assessment tab for document: "+DocumentTitle+" Admin Mode-> "+AdminMode+" Admin Series-> "+AdminSeries+" and difference value is: "+PassPerc); 	
			}else{
