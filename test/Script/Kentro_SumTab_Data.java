...
public class Kentro_SumTab_Data {
...
//		System.out.println("Total taken students count in summary tab:" +SumTakenStudent);
		...
		for(int s=0;s<sumTab.length;s++){
			sumValues[s] =table_element1.findElement(By.xpath("tbody/tr[4]/td["+(s+1)+"]")).getText();
			String sum  = table_element1.findElement(By.xpath("tbody/tr[5]/td["+(s+1)+"]")).getText();
			sumPerc[s]=Double.parseDouble(sum);
...
//	    System.out.println("-->"+Arrays.toString(SumGender_Ethn_Task));
		...
		Sum_TaskCount = Ints.toArray(list);
