...
*/			List<WebElement> ele1 = login.findElements(By.id("AccountabilityAssessmentStudentMastery"));	        
...
//					System.out.println(latsetVal);
					...
					List<WebElement> cl = login.findElements(By.xpath("//colorlist[6]"));
                    ...
                    for(int i=0;i < 2;i++){
                    WebElement web1=t.findElement(By.xpath("thead/tr/th["+(w+2)+"]"));
    			    web1.findElement(By.tagName("selectedcolor")).click();
					System.out.println("clicked11..");
    			    Thread.sleep(2000);
					LS_Task[i] = login.findElement(By.xpath("//colorlist[6]/color["+(i+1)+"]")).getText();
				//	System.out.println("Latest Staar options: "+LS_Task[i]);
					login.findElement(By.xpath("//colorlist[6]/color["+(i+1)+"]")).click();
				    Thread.sleep(5000);
					String DQ = login.findElement(By.className("reccount")).getText();
				    Stud_LS_Records[i] = Integer.parseInt(DQ.replace(" Records", ""));
					System.out.println("Value: "+LS_Task[i]+" and Stud_DQ_Records: "+Stud_LS_Records[i]); 
					Thread.sleep(2000);
					}
				}					
			}	      
...
/*	      for(int task=0; task < StudPF_Tasks.length; task++){
 		  ...
 		  System.out.println("verifed the data..");
	}		
  }
}
