...
public class STAAR_DemoTab {
	...
	public void VerifyProgramFunInfo(String DocTitle) throws InterruptedException{
		...
		if(RecCount.equalsIgnoreCase("0 Records")){
			...
			List<WebElement> Demo = login.findElements(By.id("MaintenanceAssessmentDemogBreakdown"));	            	
			WebElement DemoData = Demo.get(1);	       
				...
				for(int j=1; j < SumGender.length+1; j++){	
					...
					if(DemoGenderTask.equalsIgnoreCase(SumGenderTask)){
						if(Arrays.equals(GenderRowValue1[i-1], SumGender_val[j-1])){
							...
							debugLog.error("Did not match Demography gender task: "+DemoGenderTask+" and Summary gender task: "+ SumGenderTask);
						}
						if(DemoGen_NotTakenCount == SumGen_NotTakenCount){
							...
							debugLog.error(DemoGenderTask+"--"+SumGenderTask+" gender task not taken student count is not matching for document: " +DocTitle+ "Expected value on Student tab is "+DemoGen_NotTakenCount+" and found value on summary tab is: "+SumGen_NotTakenCount);
						}
						if(DemoGen_TakenCount == SumGen_TakenCount){
							...
							debugLog.error(DemoGenderTask+"--"+SumGenderTask+" gender task taken student count is not matching for document: " +DocTitle+ "Expected value on Student tab is "+DemoGen_TakenCount+" and found value on summary tab is: "+SumGen_TakenCount);
						}
					}
				}
			}
				...
				for(int j=0;j<SumEthnicity.length;j++){
					...
					if(DemoEthTask.equalsIgnoreCase(SumEthTask)){	
						...
						if(DemoEth_NotTakenCount == SumEth_NotTakenCount){
							...
							debugLog.error(DemoEthTask+"--"+SumEthTask+" ethnicity not taken student count is not matching for document: " +DocTitle+ " Expected value on Student tab is "+DemoEth_NotTakenCount+" and found value on summary tab is: "+SumEth_NotTakenCount);
						}
						if(DemoEth_TakenCount == SumEth_TakenCount){
							...
							debugLog.error(DemoEthTask+"--"+SumEthTask+" ethnicity taken student count is not matching for document: " +DocTitle+ " Expected value on Student tab is "+DemoEth_TakenCount+" and found value on summary tab is: "+SumEth_TakenCount);
						}
					}
				}
			}
				...
				for(int j=1; j < SumPF_Tasks.length; j++){	
					...
					if(DemoTask.equalsIgnoreCase(SumTask)){
						...
						if(Arrays.equals(rowValue1[i-1], SumPF_val[j-1])){
							...
							debugLog.error("Did not match Demography PF task: "+DemoTask+" and Summary PF task: "+ SumTask);
						}		            	
						if(DemoPF_NotTakenCount == SumPF_NotTakenCount){
							...
							debugLog.error(DemoTask+"--"+SumTask+"PF not taken student count is not matching for document: " +DocTitle+ "Expected value on Student tab is "+DemoPF_NotTakenCount+" and found value on summary tab is: "+SumPF_NotTakenCount);
						}
						if(DemoPF_TakenCount == SumPF_TakenCount){
							reportsLog.info("PF taken student count is matching for task: "+DemoTask+" and "+SumTask);
						}else{
							debugLog.error(DemoTask+"--"+SumTask+" PF taken student count is not matching for document: " +DocTitle+ "Expected value on Student tab is "+DemoPF_TakenCount+" and found value on summary tab is: "+SumPF_TakenCount);
						}
